...
Um detalhe comportamental relevante distingue o filtro de UF dos demais: ao contrário dos seletores de Ano e Mês — que inicializam com todas as opções pré-selecionadas por defeito (\texttt{default=options}) —, o filtro de UF não possui valor padrão, iniciando vazio. As três abas de dados (Lista dos Dados Armazenados, Estatísticas Descritivas e Gráficos Analíticos) exibem uma mensagem informativa enquanto nenhuma UF for selecionada, prevenindo a execução de consultas sem qualquer filtro geográfico e potencialmente custosas sobre a totalidade das tabelas fato.
