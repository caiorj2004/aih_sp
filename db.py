...
def load_scatter_data(
    ...
    """
    Retorna uma linha por município com as somas de todas as colunas de
    procedimento (qtd_* e vl_*) já agregadas no PostgreSQL via GROUP BY.

    Diferente de load_consolidated_data — que retorna uma linha por
    (município × período) — esta função consolida todos os períodos
    selecionados em uma única linha por município. Isso reduz o volume
    transferido de O(municípios × períodos) para O(municípios), tornando
    o scatter plot viável mesmo com todas as UFs selecionadas.

    Colunas retornadas:
        cod_municipio, municipio_nome, uf_codigo, uf_sigla, uf_nome,
        total_qtd, total_vl, qtd_*, vl_*  (todas como FLOAT8)
    """
    try:
        conn = get_connection()
        mapping = get_dimension_mapping()
        ...
        # SUM() de cada coluna de procedimento diretamente no SQL
        qtd_agg = ", ".join(
            f'SUM(CAST(q."{c}" AS FLOAT8)) AS "{c}"'
            for c in _QTD_PROC_COLS
            if c in real_qtd_cols
        )
        vl_agg = ", ".join(
            f'SUM(CAST(v."{c}" AS FLOAT8)) AS "{c}"'
            for c in _VL_PROC_COLS
            if c in real_vl_cols
        )
        proc_agg_parts = [p for p in (qtd_agg, vl_agg) if p]
        proc_agg_clause = (", " + ", ".join(proc_agg_parts)) if proc_agg_parts else ""

        params: Dict[str, str] = {}
        y_clause  = _build_in_clause("q.ano", selected_years,  "yr", params)
        m_clause  = _build_in_clause("q.mes", selected_months, "mo", params)
        uf_clause = _build_in_clause("CAST(m.uf_codigo AS TEXT)", selected_ufs, "uf", params) \
                    if selected_ufs else ""
        ...
        query = (
            ...
            f"  f.uf_sigla  AS uf_sigla,"
            f"  f.uf_nome   AS uf_nome,"
            f"  SUM(CAST(q.total AS FLOAT8)) AS total_qtd,"
            f"  SUM(CAST(v.total AS FLOAT8)) AS total_vl"
            f"  {proc_agg_clause}"
            f" FROM aih_qtd q"
            f" INNER JOIN aih_vl v"
            f"   ON v.ano = q.ano AND v.mes = q.mes"
            ...
            f" GROUP BY q.cod_municipio, f.nome_mun, f.uf_codigo, f.uf_sigla, f.uf_nome"
            f" ORDER BY total_vl DESC"
        )
